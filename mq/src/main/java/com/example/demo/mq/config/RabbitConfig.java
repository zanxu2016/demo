...
//            .withArgument("x-dead-letter-exchange", PROCESS_EXCHANGE)
            .deadLetterExchange(PROCESS_EXCHANGE)// 上一行代码可以使用此行代码替换，语义更清晰
            // 延迟队列需要设置的消息过期后会发往的路由键名称
//            .withArgument("x-dead-letter-routing-key", DELAY_ROUTING_KEY)
            .deadLetterRoutingKey(DELAY_ROUTING_KEY)// 上一行代码可以使用此行代码替换，语义更清晰
            .build();
    }
...
//            .withArgument("x-message-ttl", 30000)
            .deadLetterExchange(PROCESS_EXCHANGE)
